...
// T.C :- O(N2), where N = size of the given array. Reason: Here, we are using nested loops to count occurrences of every element between 1 to N.
...
// T.C :- O(2N), where N = the size of the given array. Reason: We are using two loops each running for N times. So, the time complexity will be O(2N).
// S.C :- O(N) as we are using a hash array to solve this problem.


// Optimal Approach (MATHS)

// INTUITION :- The idea is to convert the given problem into mathematical equations. Since we have two variables i.e. missing and repeating, we will try to form two linear equations. And then we will find the values of two variables using those equations.
...
// T.C :- O(N), where N = the size of the given array. Reason: We are using only one loop running for N times. So, the time complexity will be O(N)
...
// INTUITION:- Assume the repeating number to be X and the missing number to be Y.
...
// T.C:- O(N), where N = the size of the given array. Reason: We are just using some loops running for N times. So, the time complexity will be approximately O(N).
// S.C:-  O(1) as we are not using any extra space to solve this problem.
